...
function getOAuthSignature(xhr, oauthTimestamp, consumerKey, params)
{
	var parsedParams = ''; 
	if (params !== 'undefined') {
		for (param in params) {
	...
	var url = xhr.location;
	if (url.indexOf("?")>-1){
		url = url.substr(0,url.indexOf("?"));
	}
	var postedParams = (xhr.connectionType == 'POST' && parsedParams) ? parsedParams.substr(1) + '&' : '';
	// var postedParams = ((xhr.connectionType == 'POST' && parsedParams) ? '&' + Ti.Network.encodeURIComponent(parsedParams + '&') : '&');
	var getParams = ((xhr.connectionType == 'GET') ? parsedParams : '');
	var base = xhr.connectionType + '&' +
		Ti.Network.encodeURIComponent(url) + '&' +
		Ti.Network.encodeURIComponent(postedParams + 'oauth_consumer_key=' + consumerKey +
			'&oauth_nonce=nonce' + oauthTimestamp +
			'&oauth_signature_method=HMAC-SHA1&oauth_timestamp=' + oauthTimestamp + getParams);
	Ti.API.info(base);
	return CryptoJS.enc.Base64.stringify(CryptoJS.HmacSHA1(base, consumerSecret + '&'));
}

function getOAuthHeader(xhr, params)
{
	var consumerKey = Ti.App.Properties.getString("consumerKey", '');
	var oauthTimestamp = String(new Date().getTime());
	oAuthHeader = 'OAuth oauth_consumer_key=' + consumerKey + //oauthConsumerKey +
		',oauth_timestamp=' + oauthTimestamp +
		',oauth_signature_method=HMAC-SHA1' +
		',oauth_signature=' + getOAuthSignature(xhr, oauthTimestamp, consumerKey, params) +
		',oauth_nonce=nonce' + oauthTimestamp;
