	...
	getEventsRequest.onload = function()
	{
		...
		if (response.success) {
			...
			for (var i=0; i<events.length; i++) 
			{
				...
				row.descriptionKey = events[i].description;
				row.openingDate = events[i].openingDate;
				row.add(imageView);
				row.id=events[i].id;
				...
				row.attending = events[i].attending;
			...
			eventsTableView.addEventListener('click', function(e) {
				openNestedWindow(Titanium.Filesystem.resourcesDirectory + 'main_windows/events/event.js', {
					...
					photo: e.row.photo,
					description: e.row.descriptionKey,
					openingDate: e.row.openingDate,
					attending: e.row.attending
				});
			});
